...
function Devis() {
    ...
    const onSubmit = (data) => {
        ...
        try {
            ...
            // Mettre un time de 10s pour reset a null
        } catch (error) {
            setSubmissionStatus('error');
        }
    };
    ...
    return (
        ...
        {submissionStatus === null && (
            <form onSubmit={handleSubmit(onSubmit)} noValidate className="w-full flex flex-col items-center gap-2 sm:gap-4 lg:gap-8 text-sm sm:text-base md:text-lg lg:text-xl 2xl:text-2xl opacity-0 animate-slide-in-right" style={{ animationDelay: '800ms' }} action="" method="post">
...
// Time pour enlever le message
// Reafficher le formulaire sans les donnees ?
// Reafficher le formulaire sans l'effet d'ouverture de la page 